       ...
       public function createCate($name)
{
    ...
    return "created";
        ...
        public function updateCate($id, $data){
            $sql = "UPDATE categories SET name = ?, is_active = ? WHERE id = ?";
            $stmt = $this->conn->prepare($sql);
            return $stmt->execute([
                $data['name'],
                $data['is_active'],
    ...
    //    khối phục danh mục
        public function restore($id){
            $sql = "UPDATE categories SET is_active = 1 WHERE id = ?";
            $stmt = $this->conn->prepare($sql);
            return $stmt->execute([$id]);
        }
        // lấy tất cả danh mục đang hoạt động

        public function getAllCate($keyword = "", $page = 1, $limit = 10){
            $page = (int)$page;
            $limit = (int)$limit;
            $offset = ($page - 1) * $limit;
            ...
            $sql = "SELECT id, name, is_active FROM {$this->table} $where ORDER BY id DESC LIMIT $limit OFFSET $offset";
            $stmt = $this->conn->prepare($sql);
            $stmt->execute($params);
